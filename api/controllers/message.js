...
function getReceivedMessages(req, res){
    ...
    Message.find({
        receiver: userId
    }).populate('emitter', 'name surname _id').paginate(page, itemsPerPage, (err, messages, total) => {
        if(err) return res.status(500).send({
            message: 'Error en la petición'
        });
...
function getEmmitMessages(req, res){
    ...
    Message.find({
        emitter: userId
    }).populate('emitter receiver', 'name surname _id').paginate(page, itemsPerPage, (err, messages, total) => {
        if(err) return res.status(500).send({
            message: 'Error en la petición'
        });
